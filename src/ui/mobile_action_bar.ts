...
export function initMobileActionBar(
  ...
  const colorPicker = createColorPicker({
    ...
    position: (anchor, popup) => {
      ...
      let top = anchorRect.top - popupHeight - 8;
  ...
  const strokeMoreBtn =
    sheet.querySelector<HTMLButtonElement>('#mss-stroke-more')!;
  updateCustomColorButton(strokeMoreBtn, customStroke, t('moreColors'));
  strokeMoreBtn.addEventListener('click', () => {
    const currentColor =
      customStroke ?? history.present.appState.strokeColor ?? '#e2e2ef';
    colorPicker.open(strokeMoreBtn, currentColor, ({ pick, preview }) => {
      if (preview)
      ...
      if (pick) {
        if (pick === 'transparent') {
          ...
          updateCustomColorButton(strokeMoreBtn, null, t('moreColors'));
        } else {
          customStroke = pick;
          saveCustomColor('stroke', pick);
          updateCustomColorButton(strokeMoreBtn, pick, t('moreColors'));
  ...
  updateCustomColorButton(fillMoreBtn, customFill, t('moreColors'));
  fillMoreBtn.addEventListener('click', () => {
    const currentColor =
      customFill ?? history.present.appState.fillColor ?? 'transparent';
    const base = currentColor === 'transparent' ? '#ffffff' : currentColor;
    colorPicker.open(fillMoreBtn, base, ({ pick, preview }) => {
      if (preview)
      ...
      if (pick) {
        if (pick === 'transparent') {
          ...
          updateCustomColorButton(fillMoreBtn, null, t('moreColors'));
        } else {
          customFill = pick;
          saveCustomColor('fill', pick);
          updateCustomColorButton(fillMoreBtn, pick, t('moreColors'));
  ...
  if (mssWidthInput.type !== 'number') {
    ...
    mssWidthInput.addEventListener('change', () => {
      const v = Math.max(
        1,
        Math.min(100, Math.round(Number(mssWidthInput.value))),
      );
  ...
  function sync(): void {
    ...
    if (!hasSelection) {
      // No selection: sync panel with appState defaults so user can set style for next stroke
      const { appState } = scene;

      sheet
        .querySelectorAll<HTMLButtonElement>(
          '#mss-stroke-swatches .cp-color-swatch',
        )
        .forEach((sw) => {
          sw.classList.toggle('active', sw.title === appState.strokeColor);
        });
      strokeMoreBtn.classList.toggle(
        'active',
        !!customStroke && customStroke === appState.strokeColor,
      );

      sheet
        .querySelectorAll<HTMLButtonElement>(
          '#mss-fill-swatches .cp-color-swatch',
        )
        .forEach((sw) => {
          sw.classList.toggle(
            'active',
            sw.title === appState.fillColor ||
              (sw.classList.contains('cp-color-swatch-transparent') &&
                appState.fillColor === 'transparent'),
          );
        });
      fillMoreBtn.classList.toggle(
        'active',
        !!customFill && customFill === appState.fillColor,
      );

      sheet
        .querySelectorAll<HTMLButtonElement>('#mss-width-presets .cp-btn')
        .forEach((btn) => {
          btn.classList.toggle(
            'active',
            btn.dataset.value === String(appState.strokeWidth),
          );
        });
      if (mssWidthDisplay)
        mssWidthDisplay.textContent = `${appState.strokeWidth}px`;
      else mssWidthInput.value = String(appState.strokeWidth);

      sheet
        .querySelectorAll<HTMLButtonElement>('#mss-style-presets .cp-btn')
        .forEach((btn) => {
          btn.classList.toggle(
            'active',
            btn.dataset.style === (appState.strokeStyle ?? 'solid'),
          );
        });

      sheet
        .querySelectorAll<HTMLButtonElement>('#mss-roughness-presets .cp-btn')
        .forEach((btn) => {
          btn.classList.toggle(
            'active',
            btn.dataset.roughness === String(appState.roughness ?? 0),
          );
        });

      const pctNo = Math.round(appState.opacity * 100);
      opacitySlider.value = String(pctNo);
      opacityVal.textContent = String(pctNo);

      // Show stroke/width/style/opacity only — no fill, corners, roughness for generic tool mode
      (
        sheet
          .querySelector('#mss-stroke-swatches')!
          .closest('.cp-section') as HTMLElement
      ).style.display = '';
      (
        sheet
          .querySelector('#mss-fill-swatches')!
          .closest('.cp-section') as HTMLElement
      ).style.display = 'none';
      (
        sheet
          .querySelector('#mss-width-presets')!
          .closest('.cp-section') as HTMLElement
      ).style.display = '';
      (
        sheet
          .querySelector('#mss-style-presets')!
          .closest('.cp-section') as HTMLElement
      ).style.display = '';
      (
        sheet
          .querySelector('#mss-roughness-presets')!
          .closest('.cp-section') as HTMLElement
      ).style.display = 'none';
      (
        sheet
          .querySelector('#mss-border-presets')!
          .closest('.cp-section') as HTMLElement
      ).style.display = 'none';
      return;
    }
    ...
    strokeMoreBtn.classList.toggle(
      'active',
      !!customStroke && customStroke === first.strokeColor,
    );
    ...
    sheet
      ...
      .forEach((sw) => {
        sw.classList.toggle(
          'active',
          sw.title === first.fillColor ||
            (sw.classList.contains('cp-color-swatch-transparent') &&
              first.fillColor === 'transparent'),
        );
      });
    fillMoreBtn.classList.toggle(
      'active',
      !!customFill && customFill === first.fillColor,
    );
    ...
    sheet
      .querySelectorAll<HTMLButtonElement>('#mss-border-presets .cp-btn')
      .forEach((btn) => {
        btn.classList.toggle(
          'active',
    ...
    (
      sheet
        .querySelector('#mss-stroke-swatches')!
        .closest('.cp-section') as HTMLElement
    ).style.display = allImage ? 'none' : '';
    (
      sheet
        .querySelector('#mss-fill-swatches')!
        .closest('.cp-section') as HTMLElement
    ).style.display = hasFill ? '' : 'none';
    (
      sheet
        .querySelector('#mss-width-presets')!
        .closest('.cp-section') as HTMLElement
    ...
    (
      sheet
        .querySelector('#mss-border-presets')!
        .closest('.cp-section') as HTMLElement
    ).style.display = first.type !== 'rectangle' ? 'none' : '';
  }
