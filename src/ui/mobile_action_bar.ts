...
export function initMobileActionBar(workspace: HTMLElement, history: History): void {
  ...
  function sync(): void {
    ...
    if (!hasSelection) {
      ...
      strokeMoreBtn.classList.toggle('active', !!customStroke && customStroke === appState.strokeColor);

      sheet.querySelectorAll<HTMLButtonElement>('#mss-fill-swatches .cp-color-swatch').forEach((sw) => {
        sw.classList.toggle('active', sw.title === appState.fillColor ||
          (sw.classList.contains('cp-color-swatch-transparent') && appState.fillColor === 'transparent'));
      });
      fillMoreBtn.classList.toggle('active', !!customFill && customFill === appState.fillColor);

      sheet.querySelectorAll<HTMLButtonElement>('#mss-width-presets .cp-btn').forEach((btn) => {
        btn.classList.toggle('active', btn.dataset['value'] === String(appState.strokeWidth));
      });
      if (mssWidthDisplay) mssWidthDisplay.textContent = `${appState.strokeWidth}px`;
      ...
      sheet.querySelectorAll<HTMLButtonElement>('#mss-style-presets .cp-btn').forEach((btn) => {
        btn.classList.toggle('active', btn.dataset['style'] === (appState.strokeStyle ?? 'solid'));
      });

      sheet.querySelectorAll<HTMLButtonElement>('#mss-roughness-presets .cp-btn').forEach((btn) => {
        btn.classList.toggle('active', btn.dataset['roughness'] === String(appState.roughness ?? 0));
      });
      ...
      (sheet.querySelector('#mss-fill-swatches')!.closest('.cp-section') as HTMLElement).style.display = 'none';
      ...
      (sheet.querySelector('#mss-roughness-presets')!.closest('.cp-section') as HTMLElement).style.display = 'none';
      (sheet.querySelector('#mss-border-presets')!.closest('.cp-section') as HTMLElement).style.display = 'none';
      return;
    }
