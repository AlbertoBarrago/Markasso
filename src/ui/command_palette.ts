...
const _IC_TOOL = `<svg width="14" height="14" viewBox="0 0 20 20" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round"><rect x="3" y="5" width="14" height="10" rx="1.5"/></svg>`;
const _IC_ACTION = `<svg width="14" height="14" viewBox="0 0 20 20" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><path d="M4 8h9a4 4 0 010 8H8"/><path d="M7 5L4 8l3 3"/></svg>`;
...
export function initCommandPalette(
  ...
  function buildItems(): PaletteItem[] {
    ...
    const TOOL_DEFS: {
      ...
      },
      {
        ...
        label: t('select'),
        hint: 'V',
        icon: `<svg width="14" height="14" viewBox="0 0 20 20" fill="currentColor"><path d="M5.5 3v12.8l2.9-2.9 2.4 5.4 2.1-.95-2.4-5.4 3.8.001z"/></svg>`,
      },
      {
        id: 'rectangle',
        label: t('rectangle'),
        hint: 'R',
        icon: `<svg width="14" height="14" viewBox="0 0 20 20" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linejoin="round"><rect x="3" y="5" width="14" height="10" rx="1.5"/></svg>`,
      },
      {
        id: 'ellipse',
        label: t('ellipse'),
        hint: 'E',
        icon: `<svg width="14" height="14" viewBox="0 0 20 20" fill="none" stroke="currentColor" stroke-width="1.8"><ellipse cx="10" cy="10" rx="7.5" ry="5.5"/></svg>`,
      },
      {
        id: 'rombo',
        label: t('rhombus'),
        hint: 'D',
        icon: `<svg width="14" height="14" viewBox="0 0 20 20" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><path d="M10 3l7 7-7 7-7-7z"/></svg>`,
      },
      {
        id: 'line',
        label: t('line'),
        hint: 'A / L / 5',
        icon: `<svg width="14" height="14" viewBox="0 0 20 20"><line x1="4" y1="16" x2="16" y2="4" stroke="currentColor" stroke-width="1.8" stroke-linecap="round"/></svg>`,
      },
      {
        id: 'freehand',
        label: t('pen'),
      ...
      },
      {
        ...
        label: t('textTool'),
        hint: 'T',
        icon: `<svg width="14" height="14" viewBox="0 0 20 20" fill="currentColor"><path d="M4 4h12v2.5H12v9.5H8V6.5H4z"/></svg>`,
      },
      {
        id: 'sticky',
        label: t('stickyNote').replace(/\s*\(.*\)/, ''),
        hint: 'N',
        icon: `<svg width="14" height="14" viewBox="0 0 20 20" fill="none" stroke="currentColor" stroke-width="1.7" stroke-linecap="round" stroke-linejoin="round"><path d="M4 3h12a1 1 0 011 1v9l-4 4H4a1 1 0 01-1-1V4a1 1 0 011-1z"/></svg>`,
      },
      {
        id: 'eraser',
        label: t('eraser'),
        hint: '0',
        icon: `<svg width="14" height="14" viewBox="0 0 20 20" fill="none" stroke="currentColor" stroke-width="1.7" stroke-linecap="round" stroke-linejoin="round"><path d="M3 17h14"/><path d="M5 17l-2-4 9-8 4 4-7 8H5z"/></svg>`,
      },
    ];
    for (const td of TOOL_DEFS) {
      items.push({
        ...
        category: 'tool',
        icon: td.icon,
        execute: () => history.dispatch({ type: 'SET_TOOL', tool: td.id }),
      });
    }
  ...
  function setActive(idx: number, scroll: boolean): void {
    list.querySelectorAll<HTMLElement>('.cmd-item').forEach((el) => {
      el.classList.remove('active');
    });
    activeIdx = idx;
    if (idx < 0 || idx >= visibleItems.length) return;
    const el = list.querySelector<HTMLElement>(
      `.cmd-item[data-index="${idx}"]`,
    );
    if (el) {
